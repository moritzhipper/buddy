...
userRoute.get('/', expressAsyncHandler(async (req, res) => {
    const result = await buddyDB.one('SELECT email, secret, is_full_user FROM users WHERE id=$1', res.locals.userID, (res) => ({ ...res, secret: encodeUUID(res.secret) }));

    res.send(remapKeys(result, false));
}));

// delete account
userRoute.delete('/', expressAsyncHandler(async (req, res) => {
