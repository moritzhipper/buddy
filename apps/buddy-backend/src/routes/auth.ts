...
authRoute.post('/cred', validateReqBody(UserCredentialsSchema), expressAsyncHandler(async (req, res) => {
    ...
    await buddyDB.tx(async (t) => {
        ...
        const updateWasSuccesfull = await t.result(`UPDATE users SET password=crypt($1, gen_salt('bf')), email=$2, is_full_user=true WHERE id=$3`, [password, email, userID], result => (result.rowCount === 1));
...
// rotate Buddy-Secret 
authRoute.patch('/key', expressAsyncHandler(async (req, res) => {
...
// update password
authRoute.patch('/cred/password', validateReqBody(UserPasswordSchema), expressAsyncHandler(async (req, res) => {
    await buddyDB.tx(async t => {
        const updateSuccesfull = await buddyDB.result(`UPDATE users SET password=crypt($1, gen_salt('bf')) WHERE id=$2`, [req.body.password, res.locals.userID], res => res.rowCount === 1);
...
// update email
authRoute.patch('/cred/email', validateReqBody(UserEmailSchema), expressAsyncHandler(async (req, res) => {
    await buddyDB.tx(async t => {
        const updateSuccesfull = await buddyDB.result(`UPDATE users SET email=$1 WHERE id=$2`, [req.body.email, res.locals.userID], res => res.rowCount === 1);
        if (!updateSuccesfull) throw createHttpError(500);
...
authRoute.post('/sessions', validateReqBody(UserCredentialsSchema), expressAsyncHandler(async (req, res) => {
    const sessionID = await buddyDB.tx(async (t) => {
        ...
        `, [userID, clientIp, isFullUser, sessionID], result => result.rowCount === 1);
...
authRoute.delete('/sessions', expressAsyncHandler(async (req, res) => {
...
authRoute.delete('/sessions/all', expressAsyncHandler(async (req, res) => {
