...
import {
   ...
   UserPasswordSchema,
   validateReqBody,
} from '../types-and-schemas/validation-schemas';
...
authRoute.post(
   ...
   expressAsyncHandler(async (req, res) => {
      ...
      await buddyDB.tx(async (t) => {
         const emailAlreadyInUse = await t.one(
         ...
         const userAlreadyIsSecure = await t.one(
            'SELECT is_full_user from users where id=$1',
            userID,
            (result) => result.is_full_user
         );
         if (userAlreadyIsSecure)
            throw createHttpError(
               409,
               'Auth via mail and password already set up'
            );
...
authRoute.post(
   ...
   expressAsyncHandler(async (req, res) => {
      const sessionID = await buddyDB.tx(async (t) => {
         const { userID, isFullUser } = await getUserAuth(
...
authRoute.delete(
   ...
   expressAsyncHandler(async (req, res) => {
      await buddyDB.none(
         'DELETE FROM sessions WHERE user_id=$1 and id=crypt($2, id)',
         [res.locals.userID, getSessionIDFromRequest(req)]
      );
...
const deleteSessionsQuery = (userID) =>
   pgp.as.format(`DELETE FROM sessions WHERE user_id=$1`, userID);
