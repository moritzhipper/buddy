...
therapistsRoute.get('/', expressAsyncHandler(async (req, res) => {
...
// add therapist
therapistsRoute.post('/', validateReqBody(AddTherapistSchema), expressAsyncHandler(async (req, res) => {
    ...
    if (name) {
        therapist = await buddyDB.one(`insert into users_therapists (name, user_id) values ($1,$2) returning id`,
            [name, res.locals.userID], res => ({ id: res.id, name }));
    } else if (id) {
...
// update
therapistsRoute.patch('/:id', validateReqUUID, validateReqBody(TherapistSchema), expressAsyncHandler(async (req, res) => {
    ...
    await buddyDB.tx(async (t) => {
        const userOwnsTherapist = await t.one('select exists(select 1 from users_therapists where user_id=$1 and id=$2)', [userID, therapistID], result => !!result.exists);
...
therapistsRoute.delete('/:id', validateReqUUID, expressAsyncHandler(async (req, res) => {
...
therapistsRoute.post('/find', validateReqBody(TherapistSearchSchema), expressAsyncHandler(async (req, res) => {
...
function generateFindSharedTherapistsQuery(name: string): string {
    const query = pgp.as.format(`
        ...
        WHERE name ILIKE $1 LIMIT 9`, `%${name}%`);
...
function mergeTherapistsWithCallTimes(therapists: TherapistHavingID[], callTimes: CalltimeHavingID[]): TherapistHavingID[] {
    const selectCorrectCT = (thID, cts) =>
        cts.filter(ct => ct.therapistID === thID).map(ct => {
