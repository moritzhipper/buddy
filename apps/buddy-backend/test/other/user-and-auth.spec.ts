...
describe('User And Auth', () => {
    ...
    it('Use email + password for login', async () => {
        ...
        expect(
            await bt.agent
                .post('/auth/sessions')
                .auth(bt.email, bt.password)
                .send(),
            'Login with secret + pw'
        ).to.have.status(200);
    });
    ...
    it('Change password works correctly', async () => {
        ...
        expect(
            await bt.patch(Routes.AUTH + '/cred/password')
                .send({ password: alternativePassword }),
            'Change password request'
        ).to.have.status(200);

        expect(
            await bt.post(Routes.AUTH + '/sessions').auth(bt.email, alternativePassword).send(),
    ...
    it('Change email works correctly', async () => {
        ...
        expect(
            await bt.patch(Routes.AUTH + '/cred/email')
                .send({ email: alternativeEmail }),
            'Change email request'
        ).to.have.status(200);

        expect(
            await bt.post(Routes.AUTH + '/sessions').auth(alternativeEmail, bt.password).send(),
    ...
    it('Should clean up user related data on profile deletion', async () => {
        ...
        expect(
            await bt.post(Routes.NOTES).send(note),
            'Create note'
        ).to.have.status(200)

        const goal: Goal = { body: 'test' };
        expect(
            await bt.post(Routes.GOALS).send(goal),
            'Create goal'
        ).to.have.status(200)

        const appointment: Appointment = { from: '2:00', to: '2:00', isRepeating: false, date: '2023-12-22' };
        expect(
            await bt.post(Routes.APPOINTMENTS).send(appointment),
            'Create Appointment'
        ).to.have.status(200)

        const therapist: Therapist = { name: 'Hr. Miau' };
        expect(
            await bt.post(Routes.THERAPISTS).send(therapist),
            'Create therapist'
        ).to.have.status(200)
