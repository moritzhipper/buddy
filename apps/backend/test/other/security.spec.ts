...
describe('Auth and Security', () => {
   ...
   describe('Authorization related', () => {
      it('Session of not full user can not be used to access full user only route', async () => {
         ...
         const notesRes = await bt.agent.get(Routes.NOTES).set('Cookie', `sessionID=${sessionID}`).send()
      ...
      it('Login via QR-Key is not possible anymore after upgrade', async () => {
         ...
         const loginRes = await bt.agent.post('/auth/sessions').set('Buddy-Secret', bt.secret).send()
   ...
   describe('Session Related', () => {
      ...
      it('When user creates two session from same ip, old gets deleted and replaced', async () => {
         ...
         // create 1 session
         await bt.agent.post('/auth/sessions').set('Buddy-Secret', secret).send()

         // create second sesseion
         const sessionRes = await bt.agent.post('/auth/sessions').set('Buddy-Secret', secret).send()
         ...
         expect(await bt.agent.delete('/auth/sessions/all').set('Cookie', `sessionID=${sessionID}`).send(), 'Send DELETE to logout').to.have.status(
            200
         )
      ...
      it('Succesfully delete session on logout: DELETE /sessions', async () => {
         ...
         expect(await bt.agent.delete('/auth/sessions').set('Cookie', `sessionID=${sessionID}`).send(), 'Send DELETE to logout').to.have.status(200)
      ...
      it('Delete sessions on email change', async () => {
         ...
         expect(await bt.patch('/auth/cred/email').send({ email: newEmail })).to.have.status(200)
