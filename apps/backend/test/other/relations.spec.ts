...
describe('Relations', () => {
    ...
    before(async () => { await bt.createUser(); });
    ...
    it('Relate note to goal', async () => {
        ...
        expect(
            await bt.delete(Routes.GOALS + '/' + goalID).send(updatedNote),
            'Delete goal'
        ).to.have.status(200);
    ...
    it('Relate appointment to therapist, then succesfullly delete therapist', async () => {
        ...
        expect(
            await bt.patch(Routes.APPOINTMENTS + '/' + apptID).send(updatedAppt),
            'Update appointment'
        ).to.have.status(200);

        // delete therapist sucessfuylly
        expect(
            await bt.delete(Routes.THERAPISTS + '/' + therID).send(),
            'Delete therapist'
        ).to.have.status(200);
    });
});