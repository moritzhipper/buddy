...
describe('Appointments', () => {
    ...
    it('Adding one throws error on missing day', async () => {
        ...
        expect(
            await bt.post(Routes.APPOINTMENTS).send(apptWithoutDay)
        ).to.have.status(400);
    });
    ...
    it('Adding one throws error on missing date', async () => {
        ...
        expect(
            await bt.post(Routes.APPOINTMENTS).send(apptWithoutDate)
        ).to.have.status(400);
    });
    ...
    it('Adding one throws error on missing time', async () => {
        ...
        expect(
            await bt.post(Routes.APPOINTMENTS).send(apptWithoutTime)
        ).to.have.status(400);
    });
    ...
    it('Update appointment succesfully', async () => {
        ...
        expect(
            await bt.patch(Routes.APPOINTMENTS + '/' + appointmentID).send(updateAppt)
        ).to.have.status(200);
    ...
    it('Delete appointment succesfully', async () => {
        expect(
            await bt.delete(Routes.APPOINTMENTS + '/' + appointmentID).send(),
            'Delete Appointment'
        ).to.have.status(200);
