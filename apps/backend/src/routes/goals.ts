...
// read goals
goalsRoute.get('/', expressAsyncHandler(async (req, res) => {
...
// create
goalsRoute.post('/', validateReqBody(GoalSchema), expressAsyncHandler(async (req, res) => {
    const { body } = req.body;
...
// update
goalsRoute.patch('/:id', validateReqUUID, validateReqBody(GoalSchema), expressAsyncHandler(async (req, res) => {
    const mappedGoal = remapKeys(req.body);
    const update = pgp.helpers.update(mappedGoal, null, 'goals');

    const affectedRows = await buddyDB.result(`${update} WHERE user_id=$1 AND id=$2 `, [res.locals.userID, req.params.id], result => result.rowCount);
...
// delete
goalsRoute.delete('/:id', validateReqUUID, expressAsyncHandler(async (req, res) => {
    const affectedRows = await buddyDB.result('DELETE FROM goals WHERE user_id=$1 AND id=$2', [res.locals.userID, req.params.id], result => result.rowCount);
