...
async function getUserAuthViaCredentials(credentials: UserCredentials, t: pgPromise.ITask<{}>): Promise<UserAuth> {
   ...
   if (credentials.email) {
      queryResult = await t.oneOrNone(`SELECT id, is_full_user FROM users WHERE email=$1 AND password=crypt($2, password)`, [
         credentials.email,
         credentials.password,
      ])
   } else {
      const decodedSecret = decoodeBuddySecret(credentials.secret)
      queryResult = await t.oneOrNone(`SELECT id, is_full_user FROM users WHERE secret=$1 AND password=crypt($2, password)`, [
         decodedSecret,
         credentials.password,
      ])
   }
