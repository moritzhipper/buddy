...
export const expandAnimation = trigger('expanded', [
   ...
   transition(':leave', [animate('300ms ease', style({ height: 0, opacity: 0, transform: 'translateY(-10px)' }))]),
])
...
export const fadeInFromZeroHeight = trigger('fadeInFromZero', [
   transition(':enter', [style({ opacity: 0, height: 0 }), animate('200ms ease', style({ opacity: 1, height: '*' }))]),
   transition(':leave', [animate('200ms ease', style({ opacity: 0, height: 0, padding: 0 }))]),
])
