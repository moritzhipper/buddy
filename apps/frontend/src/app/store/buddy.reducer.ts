...
const createCrudReducer = <T extends UniqueItem>(crudActions, init?: T[]) => {
   ...
   return createReducer(
      ...
      on(crudActions.deleteSuccess, (state, { id }) =>
...
export function storeSyncReducer(
      ...
      if (action.type === INIT || action.type === UPDATE) {
         ...
         if (stateFromStorage) {
            try {
               ...
               console.error(
                  'couldnt parse store from local storage. removing entry'
               );
...
export function resetUserDataReducer(
   reducer: ActionReducer<any>
): ActionReducer<any> {
   return (state, action) => {
      if (
 ...
 * @returns []
 */
function updateArrayByID<T extends UniqueItem>(
   currentState: T[],
   newItem: T
): T[] {
   const stateHavingObWIthID = currentState.some(
      (item) => item.id === newItem.id
   );
   // if object containig id exists: update. Else just append to array
   if (stateHavingObWIthID) {
      return currentState.map((object) =>
         object.id === newItem.id ? { ...object, ...newItem } : object
      );
   } else {
