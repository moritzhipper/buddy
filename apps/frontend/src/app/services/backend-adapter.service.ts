...
export class BackendAdapterService {
   ...
   postUniqueItem<T>(resource: T, route: string): Observable<T & UniqueItem> {
      return this._httpClient.post<T & UniqueItem>(
         this.SERVICE_URL + route,
         resource
      );
   }

   patchUniqueItem<T extends UniqueItem>(
      resource: T,
      route: string
   ): Observable<T> {
      ...
      return this._httpClient
         .patch<T>(this.SERVICE_URL + route + '/' + id, resourceClone)
         .pipe(map(() => resource));
   }

   deleteUniqueItem(id: string, route: string): Observable<string> {
      return this._httpClient
         .delete(this.SERVICE_URL + route + '/' + id)
         .pipe(map(() => id));
   }
   ...
   patch<T>(resource: T, route: string): Observable<T> {
      return this._httpClient
         .patch(this.SERVICE_URL + route, resource)
         .pipe(map(() => resource));
   }

   updateSettings(settings: Settings): Observable<Settings> {
      return this._httpClient.patch(
         this.SERVICE_URL + this.ROUTE_SETTINGS,
         settings
      );
   }
   ...
   createPassword(password: string): Observable<any> {
      return this._httpClient.post(
         this.SERVICE_URL + this.ROUTE_USER_PROFILE,
         password
      );
   }
   ...
   rotateQRkey(): Observable<{ secret: string }> {
      return this._httpClient.patch<{ secret: string }>(
         this.SERVICE_URL + this.ROUTE_AUTH + '/key',
         null
      );
   }

   createSession(login: UserLogin): Observable<{ session: string }> {
      return this._httpClient.post<{ session: string }>(
         this.SERVICE_URL + this.ROUTE_AUTH + '/sessions',
         login
      );
   }

   getSuggestions(filter: any): Observable<Therapist[]> {
      return this._httpClient.post<Therapist[]>(
         this.SERVICE_URL + this.ROUTE_THERAPIST + '/find',
         filter
      );
   }
   ...
   logoutEverywhere() {
      return this._httpClient.delete(
         this.SERVICE_URL + this.ROUTE_AUTH + '/sessions/all'
      );
   }
}
