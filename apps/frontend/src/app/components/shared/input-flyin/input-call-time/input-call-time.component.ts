   ...
   animations: [
      trigger('fade', [
         ...
         transition(':enter', [
            ...
            animate(
               '200ms cubic-bezier(.41,0,0,.99)',
               style({ opacity: 0, transform: 'scale(.9)', height: '*' })
            ),
            animate(
               '200ms cubic-bezier(.41,0,0,.99)',
...
export class InputCallTimeComponent implements OnInit, AfterViewInit {
   ...
   ngOnInit(): void {
      ...
      if (this.preset) {
         this.callTimesFormArray = this._fb.array(
            this.preset.map((v) => this.createCalltimeFormElement(v))
         );
      } else {
         this.callTimesFormArray = this._fb.array([
            this.createCalltimeFormElement(),
         ]);
      }
      this.callTimesForm = this._fb.group({
         callTimes: this.callTimesFormArray,
      });
   }
   ...
   addCallTimeFormElement() {
      ...
      setTimeout(
         () =>
            scrollToBottomIfChildrenOverflow(
               this.htmlFormElements.nativeElement,
               false,
               400
            ),
         200
   ...
   submit() {
      this.callTimesForm.valid &&
         this._is.confirmValue(this.callTimesForm.value.callTimes);
   }
