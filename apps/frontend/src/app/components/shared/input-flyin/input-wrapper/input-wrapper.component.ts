...
@Component({
   ...
   animations: [
      trigger('fade', [
         transition(':enter', [style({ opacity: 0 }), group([animate('200ms', style({ opacity: 1 })), query('@flyIn', animateChild())])]),
         transition(':leave', [group([animate('200ms', style({ opacity: 0 })), query('@flyIn', animateChild())])]),
      ]),
      trigger('flyIn', [
         transition(':enter', [
         ...
         transition(':leave', [animate('200ms', style({ transform: 'translateY(20px)' }))]),
      ]),
   ],
})
export class InputWrapperComponent implements OnInit, OnDestroy {
   ...
   isSingleInput = (inputType: InputTypes) =>
      [InputTypes.TEXT_SHORT, InputTypes.NUMBER, InputTypes.TEXT_LONG, InputTypes.PHONE, InputTypes.MAIL, InputTypes.DATE].includes(inputType)
}
