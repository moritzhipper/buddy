...
export class InputTherapistComponent implements OnDestroy {
   ...
   constructor() {
      this.subscription = this.therapistForm.valueChanges
         .pipe(
            map((v) => v.therapist),
            switchMap((v) => {
               if (v.length <= 2) {
   ...
   submit() {
      this.therapistForm.valid &&
         this.is.confirmValue({ name: this.therapistForm.value.therapist });
   }
}
