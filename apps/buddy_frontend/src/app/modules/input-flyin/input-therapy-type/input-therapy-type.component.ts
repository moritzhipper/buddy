...
export class InputTherapyTypeComponent implements OnInit, AfterViewInit {
  ...
  submit(): void {
    this.therapyTypeForm.valid && this._is.confirmValue(this.getSelectedTypes(this.therapyTypeForm.value))
  }

  getSelectedTypes(formElements) {
    return Object.keys(formElements).filter(therapyType => formElements[therapyType]);
  }
