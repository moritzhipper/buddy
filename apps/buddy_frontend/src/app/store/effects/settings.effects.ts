...
export class SettingsEffects {
    ...
    loadSettingsInfoAfterLogin = createEffect(() =>
        this.actions$.pipe(
            ...
            switchMap(() => this.backendAdapter.get<Settings>(this.backendAdapter.ROUTE_SETTINGS).pipe(
    ...
    toggleAppt$ = createEffect(() =>
        this.actions$.pipe(ofType(settingsActions.toggleAppointment),
            switchMap(() => this.store.select(selectSettings).pipe(take(1))),
            map(settings => settingsActions.update({ props: { remindNextAppointment: !settings.remindNextAppointment } })))
    );

    updateSettings$ = createEffect(() =>
        this.actions$.pipe(
            ofType(settingsActions.update),
            switchMap((action) => this.backendAdapter.patch(action.props, this.route).pipe(
                map((settings) => settingsActions.save({ props: settings })),
                catchError((error) => of(httpErrorActions.handle({ error })))
            ))
        )
