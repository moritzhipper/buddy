...
export class AuthEffects {
    ...
    login$ = createEffect(() =>
        this.actions$.pipe(
            ...
            switchMap((action) => this.backendAdapter.createSession(action.login).pipe(
                map((res) => authActions.loginSuccess({ session: res.session, redirect: action.redirect })),
                catchError((error: HttpErrorResponse) => {
                    const is401 = error?.status === 401;
                    const userNeedsToProvidePassword = error?.error?.message.includes('Bitte gib');

                    if (is401 && userNeedsToProvidePassword) {
                        return of(authActions.requestPassword({ secret: action.login.secret }))
                    } else {
    ...
    logout$ = createEffect(() =>
        this.actions$.pipe(
            ...
            switchMap(() => this.backendAdapter.logout().pipe(
    ...
    goToFind = createEffect(() => this.actions$.pipe(
        ofType(authActions.loginSuccess),
        filter(action => action.redirect),
        tap(() => { this.router.navigateByUrl('/find') })
    ), { dispatch: false });


    goToLogin = createEffect(() => this.actions$.pipe(
        ofType(authActions.logoutSuccess, profileActions.profileNotExisting, profileActions.deleteProfileSuccess, authActions.handleInvalidSession, authActions.requestPassword),
