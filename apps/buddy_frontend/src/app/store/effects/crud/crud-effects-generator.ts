...
export class CrudEffectsGenerator {
    ...
    createEffect = () => createEffect(() =>
        this.actions$.pipe(
            ...
            switchMap((action) => this.backendAdapter.postUniqueItem(action.props, this.route).pipe(
    ...
    readOnLoginEffect = (fullUserOnly = true) => {
        const triggerActions = fullUserOnly
            ? [loadAllDataActions.fullUser]
            : [loadAllDataActions.fullUser, loadAllDataActions.basicUser];


        return createEffect(() =>
            this.actions$.pipe(
                ofType(...triggerActions),
                switchMap(() => this.backendAdapter.get(this.route).pipe(
    ...
    updateEffect = () => createEffect(() =>
        this.actions$.pipe(
            ...
            switchMap((action) => this.backendAdapter.patchUniqueItem(action.props, this.route).pipe(
    ...
    deleteEffect = () => createEffect(() =>
        this.actions$.pipe(
            ...
            switchMap((action) => this.backendAdapter.deleteUniqueItem(action.id, this.route).pipe(
