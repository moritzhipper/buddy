   ...
   createEffect = () =>
      createEffect(() =>
         this.actions$.pipe(
            ...
            switchMap((action) =>
               this.backendAdapter
                  .postUniqueItem(action.props, this.route)
                  .pipe(
                     map((resourceHavingID) =>
                        this.crudActions.save({ props: resourceHavingID })
                     ),
                     catchError((error) =>
                        of(httpErrorActions.handle({ error }))
                     )
                  )
            )
         )
         ...
         this.actions$.pipe(
            ...
            switchMap(() =>
               this.backendAdapter.get(this.route).pipe(
                  map((itemList) =>
                     this.crudActions.saveMany({ props: itemList })
                  ),
   ...
   updateEffect = () =>
      createEffect(() =>
         this.actions$.pipe(
            ...
            switchMap((action) =>
               this.backendAdapter
                  .patchUniqueItem(action.props, this.route)
                  .pipe(
                     map((resource) =>
                        this.crudActions.save({ props: resource })
                     ),
                     catchError((error) =>
                        of(httpErrorActions.handle({ error }))
                     )
