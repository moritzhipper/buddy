...
export class ProfileEffects {
    ...
    createSecret$ = createEffect(() =>
        this.actions$.pipe(
            ...
            switchMap(() => this.backendAdapter.post<{ secret: string }>(this.routeUser, null).pipe(
                map((res) => profileActions.createSecretSuccess({ login: { secret: res.secret } })),
    ...
    createCredentialsSuccess$ = createEffect(() =>
        this.actions$.pipe(
            ofType(profileActions.createCredentialsSuccess, profileActions.createSecretSuccess),
    ...
    createCredentials$ = createEffect(() =>
        this.actions$.pipe(
            ...
            switchMap((action) => this.backendAdapter.post(this.routeAuth + '/cred', action.login).pipe(
                map(() => profileActions.createCredentialsSuccess({ login: action.login })),
                catchError((error) => of(httpErrorActions.handle({ error })))
            ))
        )
    );
    ...
    rotateSecret$ = createEffect(() =>
        this.actions$.pipe(
            ...
            switchMap(() => this.backendAdapter.rotateQRkey().pipe(
                map((res) => authActions.login({ login: { secret: res.secret } })),
                catchError((error) => of(httpErrorActions.handle({ error })))
            ))
        )
    );
    ...
    loadProfileInfoAfterLogin = createEffect(() =>
        this.actions$.pipe(
            ...
            switchMap(() => this.backendAdapter.get<UserProfile>(this.backendAdapter.ROUTE_USER_PROFILE).pipe(
