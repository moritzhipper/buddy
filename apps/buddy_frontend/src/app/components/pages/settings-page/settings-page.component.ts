   ...
   changeCalltimeRimender(preset: number) {
      this._inputService
         .openInputDialogue({
            ...
            description:
               'Wie viel Zeit willst du haben, um dich auf den Anruf vorzubereiten?',
   ...
   changeAppointmentreminder(preset: number) {
      this._inputService
         .openInputDialogue({
            ...
            description:
               'Wie viele Minuten vor Beginn der Therapiesitzung willst du erinnert werden?',
            type: InputTypes.NUMBER,
            label: 'Minuten',
            preset,
         })
   ...
   deleteAllUserData() {
      ...
      this._inputService
         .openInputDialogue({
            ...
            description:
               'Diese Aktion wirst du nicht rückgangig machen können.',
      ...
      this.userProfile$
         .pipe(
            ...
            switchMap((userProfile) =>
               from(navigator.clipboard.writeText(userProfile.secret))
            )
         )
         .subscribe(() =>
            this._toastService.sendToast({
   ...
   logout() {
      this._inputService
         .openInputDialogue({
            header: 'Abmelden',
            description:
               'Deine Daten sind danach weiterhin über den QR-Key abrufbar.',
            type: InputTypes.CONFIRM,
         })
         .then((v) => {
            if (v.type === InputResolveTypes.CONFIRM) {
