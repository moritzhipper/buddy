...
import {
   ...
   noteActions,
   settingsActions,
} from 'apps/buddy_frontend/src/app/store/buddy.actions';
import {
   selectAppointments,
   selectGoals,
   selectNotes,
   selectSettings,
   selectUserProfile,
} from 'apps/buddy_frontend/src/app/store/buddy.selectors';
import { BuddyState } from 'apps/buddy_frontend/src/app/store/buddy.state';
import {
   isAppointmentInPast,
   remindableComparator,
} from 'apps/buddy_frontend/src/app/utiles-time';
...
import {
   ...
   InputTypes,
   InputResolveTypes,
} from 'apps/buddy_frontend/src/app/services/input.service';
...
export class TherapyPageComponent {
   ...
   constructor(
      ...
      this.appointmentsReminderIsOn$ = this._store
         .select(selectSettings)
         .pipe(map((settings) => settings.remindNextAppointment));

      this.isFullUser$ = this._store
         .select(selectUserProfile)
         .pipe(map((userProfile) => !!userProfile?.isFullUser));

      // hier diese ordnung einbauen
      // timeFromNowComparator
      this.orderedAppointments$ = this.appointments$.pipe(
         map((appointments: Appointment[]) =>
            [...appointments].sort(remindableComparator)
         )
      );

      this.futureAppointments$ = this.orderedAppointments$.pipe(
         map((appt) => appt.filter((appt) => !isAppointmentInPast(appt)))
      );
      this.pastAppointments$ = this.orderedAppointments$.pipe(
         map((appt) => appt.filter(isAppointmentInPast))
      );
      ...
      this.pageIsEmpty$ = combineLatest([
         this.notes$,
         this.goals$,
         this.appointments$,
      ]).pipe(
         map(
            (pageDataArrays: any[]) =>
               pageDataArrays.filter((subArray) => subArray.length != 0)
                  .length === 0
         )
      );
   }
   ...
   addTopic() {
      this._inputService
         .openInputDialogue({
            ...
            description:
               'Halte eine Thematik fest, die du in dieser Therapie noch ansprechen möchtest',
         })
         .then(
            (v) =>
               v.type === InputResolveTypes.CONFIRM &&
               this._store.dispatch(
                  goalActions.create({ props: { body: v.value } })
               )
         );
   }
   ...
   editTopic(topic: Goal) {
      this._inputService
         ...
         .then((v) => {
            v.type === InputResolveTypes.CONFIRM &&
               this._store.dispatch(
                  goalActions.update({ props: { id: topic.id, body: v.value } })
               );
            v.type === InputResolveTypes.DELETE &&
               this._store.dispatch(goalActions.delete({ id: topic.id }));
         });
   }
      ...
      this._inputService
         ...
         .then(
            (v) =>
               v.type === InputResolveTypes.CONFIRM &&
               this._store.dispatch(
                  noteActions.create({ props: { body: v.value } })
               )
         );
   }
