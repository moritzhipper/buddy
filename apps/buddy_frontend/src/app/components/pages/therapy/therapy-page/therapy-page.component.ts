...
export class TherapyPageComponent {
  ...
  constructor(private _inputService: InputService, private _store: Store<BuddyState>) {
    this.appointmentsReminderIsOn$ = this._store.select(selectSettings).pipe(
      map(settings => settings.remindNextAppointment)
    );
    ...
    this.orderedAppointments$ = this.appointments$.pipe(
      map((appointments: Appointment[]) => [...appointments].sort(remindableComparator))
    );
    ...
    this.nextAppointment$ = this.futureAppointments$.pipe(filter(appt => appt?.length > 0), map(appt => appt[0]))
    ...
    this.pageIsEmpty$ = combineLatest([this.notes$, this.goals$, this.appointments$]).pipe(
      map((pageDataArrays: any[]) =>
        pageDataArrays.filter(subArray => subArray.length != 0).length === 0
      ));
  }
