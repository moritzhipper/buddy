...
import { Auth, ToastType, UserLogin, UserProfile } from "apps/buddy_frontend/src/app/models";
import { ToastService } from "apps/buddy_frontend/src/app/services/toast.service";
import { authActions } from "apps/buddy_frontend/src/app/store/buddy.actions";
import { selectAuth, selectUserProfile } from "apps/buddy_frontend/src/app/store/buddy.selectors";
...
export class LoginPageComponent implements OnDestroy {
  @ViewChild('webcamPreview', { static: false }) webcamPreview: ElementRef<HTMLVideoElement>;
  ...
  resolveScreenshot(image: File) {
    QrScanner.scanImage(image, {})
      .then(result => {
        ...
        this._toastService.sendToast({ text: 'QR-Key konnte nicht erkannt werden. Versuche es bitte erneut', type: ToastType.ERROR })
