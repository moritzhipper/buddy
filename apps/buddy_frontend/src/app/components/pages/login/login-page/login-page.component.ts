...
export class LoginPageComponent implements OnDestroy {
   ...
   allLoginOptionsClosed() {
      return !(
         this.scannerIsOpen ||
         this.screenshotSelectorIsOpen ||
         this.qrKeyInputIsOpen
      );
   }
   ...
   toggleScanner(setOpen?: boolean) {
      this.scannerIsOpen =
         setOpen !== undefined ? setOpen : !this.scannerIsOpen;

      if (this.scannerIsOpen) {
         setTimeout(() => {
            this.scanner = new QrScanner(
               this.webcamPreview.nativeElement,
               (v) => this.resolveScan(v),
   ...
   private loginViaQRKey(qrKey: string): void {
      debugger;
      this.store.dispatch(
         authActions.login({ login: { secret: qrKey }, redirect: true })
      );
   }
