...
import {
   fadeInAnimation,
   fadeOutAnimation,
} from 'apps/buddy_frontend/src/app/animations';
...
export class FindPageComponent {
   ...
   hasTherapists$ = this.therapists$.pipe(
      map((therapists) => therapists?.length > 0)
   );
   ...
   private isOneCallable(therapistList: Therapist[]) {
      return therapistList.some((therapist) =>
         therapist?.callTimes?.some(isCurrentTimeInRange)
      );
   }
}
