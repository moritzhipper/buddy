...
import {
   ...
   Therapist,
   WeekdaysMap,
} from 'apps/buddy_frontend/src/app/models';
...
export class TherapistCalendarComponent {
   ...
   nowCallableItems$ = this.calendarItems$.pipe(
      map((items) =>
         items.filter((item) => isCurrentTimeInRange(item.callTime))
      )
   );
   hasNowCallableItems$ = this.nowCallableItems$.pipe(
      map((items) => items?.length > 0)
   );

   calendarItemsByDays$: Observable<
      { weekday: string; items: CalendarItem[] }[]
   > = this.calendarItems$.pipe(
      map((calendarItems) => {
         ...
         const structuredArray = weekdays.map((weekday) => ({
            ...
            items: calendarItems.filter(
               (item) => item.callTime.weekday === weekday
            ),
         }));
   ...
   private flatMapToCalendarItems(therapists: Therapist[]): CalendarItem[] {
      const callableTherapists = therapists.filter(
         (therapist) => therapist.callTimes
      );
