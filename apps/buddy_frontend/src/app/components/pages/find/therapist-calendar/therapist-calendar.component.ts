...
import { CalendarItem, Therapist, WeekdaysMap } from "apps/buddy_frontend/src/app/models";
import { selectTherapists } from "apps/buddy_frontend/src/app/store/buddy.selectors";
import { isCurrentTimeInRange, remindableToDate } from "apps/buddy_frontend/src/app/utiles-time";
...
export class TherapistCalendarComponent {
  ...
  calendarItems$ = inject(Store).select(selectTherapists).pipe(
    map(ther => this.flatMapToCalendarItems(ther))
  );
  ...
  private flatMapToCalendarItems(therapists: Therapist[]): CalendarItem[] {
    ...
    return callableTherapists
      .flatMap(therapist => therapist.callTimes
        .map(callTime => ({
          callTime,
