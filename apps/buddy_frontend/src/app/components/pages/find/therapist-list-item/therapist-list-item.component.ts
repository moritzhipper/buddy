...
import { isCurrentTimeInRange, remindableComparator } from "apps/buddy_frontend/src/app/utiles-time";
...
@Component({
  ...
  animations: [
    ...
    trigger('fade', [
      ...
      transition(':leave', [
        animate('100ms ease-in', style({ opacity: 0, transform: 'translate(-20px, -20px)' }))
      ])
    ])
  ]
})
export class TherapistListItemComponent implements OnDestroy {
  ...
  constructor(private _renderer: Renderer2, public inputservice: InputService, private _store: Store<BuddyState>, private _toastService: ToastService) {
    this.subscription = inject(Store).select(selectUserProfile).subscribe(profile => {
  ...
  editName() {
    this.inputservice.openInputDialogue({
      header: 'Name bearbeiten', type: InputTypes.TEXT_SHORT, preset: this.therapist.name, label: 'Name'
    }).then(v => this._updateValueResolve(v, 'name'));
  }

  editNote() {
    this.inputservice.openInputDialogue({
      header: 'Notiz', type: InputTypes.TEXT_LONG, description: 'zu ' + this.therapist.name, preset: this.therapist.note, canRemove: !!this.therapist.note
    }).then(v => this._updateValueResolve(v, 'note'));
  }

  editPhone() {
    this.inputservice.openInputDialogue({
      header: 'Telefonnummer angeben', type: InputTypes.PHONE, preset: this.therapist.phone, label: 'Nummer', canRemove: !!this.therapist.phone
    }).then(v => this._updateValueResolve(v, 'phone'));
  }

  editMail() {
    this.inputservice.openInputDialogue({
      header: 'Email angeben', type: InputTypes.MAIL, label: 'Email', preset: this.therapist.email, canRemove: !!this.therapist.email
    }).then(v => this._updateValueResolve(v, 'email'));
  }

  editFreeFrom() {
    this.inputservice.openInputDialogue({
      header: 'Datum angeben', type: InputTypes.DATE, description: 'ab dem ein Therapieplatz frei ist', preset: this.therapist.freeFrom, label: 'Datum', canRemove: !!this.therapist.freeFrom
    }).then(v => this._updateValueResolve(v, 'freeFrom'));
  }

  editCallTimes() {
    this.inputservice.openInputDialogue({
      header: 'Anrufzeiten ergänzen', type: InputTypes.CALL_TIMES, description: 'wann ist die Therapeut*in telefonisch erreichbar?', preset: this.therapist.callTimes, canRemove: !!this.therapist.callTimes
    }).then(v => {
  ...
  editAddress() {
    this.inputservice.openInputDialogue({
      header: 'Adresse angeben', type: InputTypes.ADDRESS, preset: this.therapist.address, canRemove: !!this.therapist.address
    }).then(v => this._updateValueResolve(v, 'address'));
  }

  editTherapyTypes() {
    this.inputservice.openInputDialogue({
      header: 'Therapiearten angeben', type: InputTypes.THERAPYTYPE, preset: this.therapist.therapyTypes, canRemove: !!this.therapist.therapyTypes
    }).then(v => this._updateValueResolve(v, 'therapyTypes'));
  }

  deleteTherapist() {
    this.inputservice.openInputDialogue({
      header: 'Therapeut*in entfernen?', description: 'Du wirst die Person nicht wieder herstellen können.', type: InputTypes.CONFIRM
    }).then(v => {
      if (v.type === InputResolveTypes.CONFIRM) {
        this._store.dispatch(therapistActions.delete({ id: this.therapist.id }))
        this._toastService.sendToast({ type: ToastType.SUCCESS, text: `${this.therapist.name} gelöscht` })
      }
    });
  }
  ...
  toggleCallTimeReminder(index: number) {
    ...
    updatedCallTime = { ...updatedCallTime, reminder: !updatedCallTime.reminder };
  ...
  private _updateValueResolve(inputResolver: InputResolver, attributeName: string) {
    if (inputResolver.type === InputResolveTypes.CONFIRM || inputResolver.type === InputResolveTypes.DELETE) {
      ...
      this._store.dispatch(therapistActions.update({ props: { ...therapist, id: this.therapist.id } }));
    }
  }
  ...
  generateContactFooter = () => [
    this.therapist.phone,
    this.therapist.email]
    .filter(v => !!v)
    .join(', ');
